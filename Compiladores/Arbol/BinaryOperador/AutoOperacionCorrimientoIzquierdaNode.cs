...
{
    public class AutoOperacionCorrimientoIzquierdaNode:BinaryOperatorNode
    {
        public override TiposBases ValidateSemantic()
        {
            ...
            if (!(Derecho is IntTipo))
                throw new SemanticoException(" El incremeto debe ser de tipo int" + OperadorDerecho._TOKEN.Fila + " columna +" + OperadorDerecho._TOKEN.Columna);
            if (Izquierdo is IntTipo || Izquierdo is StringTipo || Izquierdo is CharTipo)
                return Izquierdo;
            throw new SemanticoException("El tipo de dato no puede hacerse corrimiento Izquierda" + _TOKEN.Fila + " columna " + _TOKEN.Columna);
        }
